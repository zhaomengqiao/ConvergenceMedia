...
export default {
    ...
    methods: {
        ...
        initEditor() {
            ...
            if (_that.scriptTagStatus === 2 && _that.instance === null) {
                // Vue 异步执行 DOM 更新，这样一来代码执行到这里的时候可能 template 里面的 script 标签还没真正创建
                // 所以，我们只能在 nextTick 里面初始化 UEditor
                _that.$nextTick(() => {

                    // _that.instance.getActionUrl(_that.instance.getOpt('imageMaxSize'));
                    _that.instance = window.UE.getEditor(_that.randomId, _that.ueditorConfig);
                    // 绑定事件，当 UEditor 初始化完成后，将编辑器实例通过自定义的 ready 事件交出去
                    _that.instance.addListener('ready', () => {
                        _that.$emit('ready', _that.instance);
                    });
                    _that.instance._bkGetActionUrl = _that.instance.getActionUrl;
                    _that.instance.getActionUrl = function(action) {
                        if (action == 'uploadimage' || action == 'uploadscrawl' || action == 'uploadimage') {
                            return UEditorBase + '/api/fileUpload/img';

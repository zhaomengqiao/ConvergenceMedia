<template>
	<section>
			...
			<el-col :span="18">
				<el-card>
					<el-row class="toolbar" style="padding-bottom: 10px;">
						<div class="news_title"><span v-if="newExam.tfmaintype=='junshi'" style="float: left;font-size: 16px;color: red;">军事头条</span><span v-if="newExam.isabroad==1" style="float: left;font-size: 16px;color: red;">境外头条专用</span>{{newExam.contenttitle}}</div>
				  	...
				  	<el-row class='box_div' id='box_div' style="position:relative">
		...
		<!--拒绝Dialog Begin-->
		<el-dialog title="拒绝原因" v-model="noPassVisible" size="tiny" top='20%' :modal="false">
			<span>
...
<script>
	...
	export default {
		...
		methods:{
			...
			getType(){
				let para={type:this.newsType,parentId:this.parentId};

...
export default {
    ...
    created() {
        ...
        this.getRedKeyWords();
        let sp_news = localStorage.getItem('sp_news');
        sp_news = JSON.parse(sp_news);
        if (sp_news == null) {
            this.getNewExam();
        } else if (sp_news.length <= 9) {
            this.newExamList = sp_news;
            ...
            localStorage.setItem('sp_news', JSON.stringify(this.newExamList));
        }
    },
    mounted() {
        let _that = this;
    ...
    methods: {
        ...
        checkedNum(subType) {
            let para = {
                type: this.newsType, // 自媒体待审核（接口名字是这样的，不要误解）
                subType: subType
            };
    ...
    destroyed() {
        ...
        localStorage.removeItem("sp_news")
        let para = {
            queue: 'sp_news'
        };

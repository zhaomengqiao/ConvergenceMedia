        ...
        <el-table :data="tableData"
            ...
            <el-table-column label="操作" width="280" fixed="right">
                <template slot-scope="scope">
                    ...
                    <el-button type="primary" size="small" @click="openNew(scope.row)" v-if="btnAuth('open',scope.row)" >打开URL</el-button>
...
export default {
    ...
    methods: {
        ...
        getAuth() {
                ...
                contentManage.subItems.forEach(function(manage_item) {
                    if (manage_item.cname == "内容编辑") {
                        ...
                        manage_item.subItems.forEach(function(edit_item) {
                            ...
                            if (edit_item.cname == "发图集") {
                                edit_item.subItems.forEach(function(send_item) {
                                    if (send_item.cname.split("-")[0] == '发图集') {
                                        editAuthPlatform.push(send_item.cname.split("-")[1]);
                                    }
                                })
                            }
                        })
            ...
            //     if(contentToTop.subItems==null||contentToTop.subItems==''){ return false }
                    ...
                    if (manage_item.cname == "内容管理工具") {
                        ...
                        manage_item.subItems.forEach(function(edit_item) {
                            ...
                            if (edit_item.cname == "内容管理工具") {
                                edit_item.subItems.forEach(function(send_item) {
                                    if (JSON.stringify(send_item.subItems).indexOf("-图集") != -1) {
        ...
        // 置顶(图集暂无置顶)
        // toTop(data) {
        //     this.$router.push({path:'/content/recommend',query:{data: {rowkey:data.url, platfrom:this.formData.platform} ,type:'imgEdit'}});
        // },
        //分页操作

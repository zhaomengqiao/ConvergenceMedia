    ...
    <section>
        ...
        <el-table :data="tableData"
            ...
            <el-table-column prop="title" label="标题">
                <template slot-scope="scope">
            		<el-popover
						  ...
						  trigger="hover"
						  >
						  <img :src="item.src" width="100%" height="100%" v-for="item in scope.row.minibjsbeans" v-if="scope.row.minibjsbeans!=null&&scope.row.minibjsbeans!=''"/>
						</el-popover>
            ...
            <el-table-column prop="" label="阅读人" width="120" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    {{ scope.row.isread==1?scope.row.username:'' }}
                </template>
            </el-table-column>
        ...
        <el-dialog title="页面预览" v-model="replyFormVisible" size="small" top="5%" :close-on-click-modal="false">
			  <div v-html="contentPreview" class="title-content-box">
...
export default {
    ...
    methods: {
        ...
        getAuth() {
            ...
            function hasAuth(authList, url) {

            }
            // 管理
            if (authList.indexOf('content/black/yangzi/article') != -1) {
                this.auth.manage = true;
            }
            // 置顶
            if (authList.indexOf('新闻推荐-平台-扬子头条') != -1) {
                this.auth.toTop = true
            }
            // 编辑
            if (authList.indexOf('sendnews-platform-YANGZI') != -1) {
                this.auth.edit = true
            }
        },
        // 获取平台
        getOperPlatform() {
            ...
            // 根据平台和按钮初始化按钮权限（默认false）
            let platformLen = this.platformOptions.length;
            let btnLen = this.btnControls.length;
            for (var i = 0; i < platformLen; i++) {
                // 权限列表第一子集为平台value
                this.authList[this.platformOptions[i].value] = {}
                // 权限列表第二子集为按钮（默认false）
                for (var index = 0; index < btnLen; index++) {
                    this.authList[this.platformOptions[i].value][this.btnControls[index]] = false
                }
            }
        },
        // 新闻主分类

...
<section>
        ...
        <el-form :inline="true" :model="form">
            <el-row>
                <div class="pull-left">
                    ...
                    <el-form-item prop="nowAppForm">
                        <el-select v-model="form.pagenum" class="select-platform">
                            <el-option v-for="(app,index) in pagenumOptionsHasAll" :key="index" :label="app.label" :value="app.value"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item prop="appfrom">
                        <el-select v-model="form.idx" class="select-platform">
                            <el-option v-for="(app,index) in idxOptionsHasAll" :key="index" :label="app.label" :value="app.value"></el-option>
                        </el-select>
                    </el-form-item>
                ...
                <div class="pull-right">
                    <el-radio-group v-model="form.channel" size="medium" @change="changeToptype">
                        <el-radio-button :label="1">置顶到新闻</el-radio-button>
                        <el-radio-button :label="0">置顶到视频</el-radio-button>
                    </el-radio-group>
                </div>
            </el-row>
            <el-row>
                <el-select v-model="form.userName" class="select-platform">
                    <el-option v-for="(item,index) in userNameList" :key="index" :label="item.realname" :value="item.uname"></el-option>
                </el-select>
                <el-form-item>
    ...
    <el-card style="margin-top:10px;" :body-style="{paddingBottom: '0px'}">
        <el-table :data="newsData" :default-expand-all="expandAllRow" :key="key" border highlight-current-row v-loading="listLoading" row-key="id" style="width: 100%;margin: 0 auto;">
            ...
            <!--点击PV Begin-->
            <el-table-column v-for="(item,index) in tableConfigCheck" :key="index" :prop="item.prop" :label="item.label" width="100" align="center" v-if="item.prop == 'clickpv'">
            </el-table-column>
            <!--点击UV Begin-->
            <el-table-column v-for="(item,index) in tableConfigCheck" :key="index" :prop="item.prop" :label="item.label" width="100" align="center" v-if="item.prop == 'clickuv'">
            </el-table-column>
            <!--展现PV Begin-->
            <el-table-column v-for="(item,index) in tableConfigCheck" :key="index" :prop="item.prop" :label="item.label" width="100" align="center" v-if="item.prop == 'showpv'">
            </el-table-column>
            <!--展现UV Begin-->
            <el-table-column v-for="(item,index) in tableConfigCheck" :key="index" :prop="item.prop" :label="item.label" width="100" align="center" v-if="item.prop == 'showuv'">
            </el-table-column>
            <!--7天平均点击PV Begin-->
            <el-table-column v-for="(item,index) in tableConfigCheck" :key="index" :prop="item.prop" :label="item.label" width="100" align="center" v-if="item.prop == 'clickpv_avg'">
            </el-table-column>
            <!--7天平均点击UV Begin-->
            <el-table-column v-for="(item,index) in tableConfigCheck" :key="index" :prop="item.prop" :label="item.label" width="100" align="center" v-if="item.prop == 'clickuv_avg'">
            </el-table-column>
            <!--7天平均展现PV Begin-->
            <el-table-column v-for="(item,index) in tableConfigCheck" :key="index" :prop="item.prop" :label="item.label" width="100" align="center" v-if="item.prop == 'showuv_avg'">
            </el-table-column>
            <!--7天平均展现UV Begin-->
            <el-table-column v-for="(item,index) in tableConfigCheck" :key="index" :prop="item.prop" :label="item.label" width="100" align="center" v-if="item.prop == 'showpv_avg'">
            </el-table-column>
        ...
        <div class="menu_slider" :class="edit_slide_show?'':'menu_slider_hide'">
            ...
            <el-form :model="form" class="el_all_width">
                ...
                <el-form-item>
                    <el-date-picker format="yyyy-MM-dd HH:mm:ss" v-model="editTime" width="100%" type="datetimerange" placeholder="选择时间范围" @change="getEditTime" :picker-options="pickerOptions">
                    </el-date-picker>
                </el-form-item>
    ...
    <el-dialog title="请选择栏目:" :visible.sync="dialogVisibleType" :before-close="handleCloseType" :close-on-click-modal="false">
        <template>
    ...
    <el-dialog title="请选择地域:" :visible.sync="dialogVisibleArea" :before-close="handleCloseTypeArea" :close-on-click-modal="false">
        <template>
...
export default {
    ...
    data() {
        ...
        var threeDay = new Date().setHours(0,0,0,0) - 3600 * 1000 * 24 * 3;
        return {
            pickerOptions: {
                shortcuts: []
            },
    ...
    mounted() {
        ...
        this.form.endTime = this.format(new Date().setHours(23,59,59,0), 'yyyy-MM-dd HH:mm:ss');
        this.form.startTime = this.format(new Date().setHours(0,0,0,0) - 3600 * 1000 * 24 * 3,'yyyy-MM-dd HH:mm:ss');
        this.newsTopInfo();
        this.getNewsTopUser()
    },
    methods: {
        ...
        getNewsTopUser() {
            let params = {
                channel: this.form.channel,// 1新闻 0 视频
            ...
            this.pickerOptions.shortcuts = [
                {
                    text: '延长1小时',
                    onClick(picker) {
                        const end = new Date(new Date(_this.editTime[1]).getTime() + 3600 * 1000 * 1);
                        const start = new Date(_this.editTime[0]);
                ...
                },
                {
                    text: '延长2小时',
                    onClick(picker) {
                        const end = new Date(new Date(_this.editTime[1]).getTime() + 3600 * 1000 * 2);
                        const start = new Date(_this.editTime[0]);
                        picker.$emit('pick', [start, end]);
                    }
                },
                {
                    text: '延长3小时',
                    onClick(picker) {
                        const end = new Date(new Date(_this.editTime[1]).getTime() + 3600 * 1000 * 3);
                        const start = new Date(_this.editTime[0]);
                        picker.$emit('pick', [start, end]);
                    }
                },
                {
                    text: '延长4小时',
                    onClick(picker) {
                        const end = new Date(new Date(_this.editTime[1]).getTime() + 3600 * 1000 * 4);
                        const start = new Date(_this.editTime[0]);
                        picker.$emit('pick', [start, end]);
                    }
                }
        ...
        newsTopInfo() {
            ...
            getOperPlatform(param).then((res) => {
                if (res.code === '00001') {
        ...
        queryPlatDetail() {
            ...
            queryPlatDetail(para).then((res) => {
                if (res.code === '00001' && res.data) {
                    ...
                    this.sendAppfrom = res.data.appfrom;
                    this.newsTopManager();
                }
            });
        },
        ...
        dcGetPv(itemData) {
            itemData.childTable = []
            var startTime = parseTime(new Date(itemData.startTime).getTime(), '{y}{m}{d}')
            var endTime = parseTime(new Date(itemData.endTime).getTime(), '{y}{m}{d}')

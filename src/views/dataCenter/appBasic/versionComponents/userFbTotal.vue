...
export default {
    ...
    data() {
        return {
            ...
            timeCur: new Date().getTime(new Date() - 3600 * 1000 * 24 * 1),
            currentRow: null,
            tableData: [],
            tableDataTotal: [],
            labelWidth: "120px",
    ...
    methods: {
        ...
        queryCreateList() {
            ...
            this.form.ctime = timeEnd.getFullYear() + '' + (timeEnd.getMonth() + 1) + '' + timeEnd.getDate();
            // this.form.ctime = '20171111';

            // 参数字符串
            let queryParam = '&startDate=' + this.form.ctime + '&endDate=' + this.form.ctime + '&apptype=' + this.form.apptype + '&callback';

            // jsonp回调函数
            window.appdayqddatatotaluserfb = function(data) {
                if (data.code === 200) {
                    // data.datalist.forEach(function(item, index) {
            ...
            jsonp('http://tongji.021.com/datacenterapi/appdayqd/appdayqddatatotaluserfb?callbackparam=appdayqddatatotaluserfb', {
                'param': queryParam,
                'name': 'callback'
            }, function(err, data) {

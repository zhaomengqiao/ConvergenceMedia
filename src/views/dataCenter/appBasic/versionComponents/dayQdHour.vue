...
<div>
    <el-form :inline="true" :model="form">
        ...
        <el-form-item label="渠道号：">
            <el-select v-model="form.appqid" class="select-platform" filterable placeholder="支持中文/拼音搜索" @change="optionChange">
...
export default {
    ...
    methods: {
        ...
        queryCreateList() {
            ...
            // 参数字符串
            let queryParam = 'pagesize=' + this.form.pagesize + '&page=' + this.form.page + '&startDate=' + this.form.ctime + '&endDate=' + this.form.ctime + '&appversion=' + this.form.appversion + '&appqid=' + this.form.appqid + '&callback';

            // jsonp回调函数
            window.appdayqdhour = function(data) {
                if (data.code === 200) {
                    data.datalist.forEach(function(item, index) {
            ...
            jsonp('http://tongji.021.com/datacenterapi/appdayqd/appdayqdhour?callbackparam=appdayqdhour', {
                'param': queryParam,
                'name': 'callback'
            }, function(err, data) {

...
<div>
    ...
    <el-table :data="tableData2" class="roomList" highlight-current-row tooltip-effect="dark" stripe border align="center" v-loading="listLoading" :default-sort="{prop: 'dt', order: 'descending'}" style="width: 100%">
        ...
        <el-table-column label="城市" align="center" sortable>
            <template slot-scope="scope">
                <el-button @click="cityClick(scope.row)" type="text" size="middle">{{ scope.row.city }}</el-button>
            </template>
        </el-table-column>
...
export default {
    ...
    methods: {
        backClick: function() {
            this.$emit('backchange', 'sBar');
        },
        cityClick: function(row) {
            this.$emit('citychange', 'dayCity', row);
        },
        ...
        getQid: function() {
            ...
            jsonp('http://tongji.021.com/datacenterapi/appqidareaspreaddata/getqid?callbackparam=Cqid', null, function(err, data) {});
        },
        init() {
            this.form.province = this.provname;
            this.form.ctime = this.dt;
            this.form.qid = this.bqid;
            this.getQid();
            this.queryCreateList();
        },
        queryCreateList() {
            let that = this;
            // 参数字符串
            let queryParam = '&startDate=' + this.form.ctime + '&endDate=' + this.form.ctime + '&qid=' + this.qid + '&city=' + this.form.city + '&province=' + this.provname + '&apptypeid=' + this.form.apptypeid + '&callback';
            // jsonp回调函数
            ...
            jsonp('http://tongji.021.com/datacenterapi/appqidareaspreaddata/appqidareaspreadbycity?callbackparam=csdata', {
                'param': queryParam,
                'name': 'callback'
            }, function(err, data) {
